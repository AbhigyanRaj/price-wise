...
**Five AI agents propose a price. A human decides. Nothing reaches the storefront without one.**

Multi-tenant dynamic pricing intelligence, built as a technical assessment for Klypup.
...
![Tests](https://img.shields.io/badge/tests-237%20passing-2f9e44)
[![CI](https://github.com/AbhigyanRaj/price-wise/actions/workflows/ci.yml/badge.svg)](https://github.com/AbhigyanRaj/price-wise/actions/workflows/ci.yml)

**[Live demo](https://pricewise-klupup.vercel.app)** ·
[API](https://price-wise-x14b.onrender.com/healthz) ·
[Architecture](ARCHITECTURE.md) ·
[Decisions](DECISIONS.md) ·
[API contract](docs/openapi.yaml)

`admin@suvidha.test` / `Pricewise2026!`
...
| | |
|---|---|
| **Live app** | **https://pricewise-klupup.vercel.app** |
| **API** | https://price-wise-x14b.onrender.com |
...
The whole system on one canvas: six panels covering system architecture, the
path of a single recommendation, the agent pipeline, tenant isolation, the data
...
Source: [`docs/Pricewise_HLD.excalidraw`](docs/Pricewise_HLD.excalidraw) · open it in [excalidraw.com](https://excalidraw.com)

</div>

The two diagrams below are the same architecture in Mermaid, which diffs in
review and cannot drift from the repository the way an exported image does.
Four more, the data flow, the ER model, tenant isolation and the API surface,
...
bun run test            # 232 tests: 132 backend, 100 frontend
...
- **Render's free tier spins down when idle**, which would make the first
  request take about 35 seconds. A scheduled ping keeps the service warm, so
  the deployed app responds immediately. Worth knowing because it is a
  workaround, not a fix: the real answer is a paid instance.
