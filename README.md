...
> The API runs on Render's free tier, which spins down after about fifteen
> minutes of inactivity. The first request then takes roughly 35 seconds while
> it wakes. Open the [health check](https://price-wise-x14b.onrender.com/healthz)
> first if you want the app itself to feel quick.
...
- **The API sleeps.** Render's free tier spins down after about fifteen minutes
  idle and the first request afterwards takes roughly 35 seconds. Measured, not
  estimated. Hit the health check first and the app feels instant.
